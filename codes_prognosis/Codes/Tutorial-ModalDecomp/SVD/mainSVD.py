...
while True:
    param['filetype'] = input('Select the input file format (.mat, .npy, .csv, .pkl, .h5, .dcm, .folder, .sequences): ')
...
while True:
    param['SNAP'] = input('Select number of snapshots. Continue with the temporal dimension of the input: ')
    if not param['SNAP']:
        break
...
Tensors, list_data_info, _ = data_load.main(param)

print('... data loaded!')

while True:
    param['n_modes'] = input('Select number of SVD modes. Continue with 5: ')
...
while True:
    param['svd_library'] = input('Select the library to use to execute SVD (numpy, torch, torch_lower_precision) or continue with torch_lower_precision: ')
...
param['filen'] = input('Enter folder name to save the outputs or continue with default folder name: ')

if not param['filen']:
    param['filen'] = os.getcwd()

param['export_svd_data'] = input('Enter 0 to only export singular values, or 1 to also export SVD reconstructions and modes (default: 0): ')

if not param['export_svd_data']:
    param['export_svd_data'] = 0

param['export_V'] = input('Enter 0 to not export V matrices, or 1 otherwise (default: 0): ')

if not param['export_V']:
    param['export_V'] = 0

while True:
    param['decision_2'] = input('Select format of saved files (.mat, .npy) or press enter to not save: ')
...
param['interactive_plot'] = input('Introduce whether activating interactive plot (0 or 1) of SVD modes or press enter to not activate: ')
