...
# -*- coding: utf-8 -*-
"""
Created on Tue May 13 16:47:55 2025
...
param['database_path'] = '/home/ander/Escritorio/Workshop2025-2026/Results_step2/svd_reconst_data' #'/home/w460/SCRATCH/Andres/Databases/Human_ecos/Ecos_tutorials/Results_step2/svd_reconst_data'

# input('Write a path in which data are stored or press enter to introduce the tensor cylinder: ')
...
param['metadata_path'] = '/home/ander/Escritorio/Workshop2025-2026/Results_step1/Metadata_databases.xlsx' # input('Write a path to a metadata file or press enter to continue without it: ')

if not param['metadata_path']:
    param['metadata_path'] = ''

# region Param Path to save results
param['results_path'] = 'home/ander/Escritorio/Workshop2025-2026/Results_step3' # input('Write a path in which results will be saved or press enter to save in the project directory: ')
...
param['svd_library'] = 'hybrid_lower_precision' # input('Introduce library to use for SVD or press enter to continue with torch \nOptions: \nnumpy \ntorch \ntensorflow \nhybrid \nhybrid_lower_precision: ')
...
param['svd_driver'] = None # input('Introduce driver to use for the case of SVD CUDA or press enter to continue with None: ')
...
param['SNAP'] = None # input('Introduce number of snapshots or press enter to continue with a number of snapshots corresponding to the size of the temporal dimension of the input data: ')
...
param['varepsilon1'] = None # input('Introduce first tolerance (SVD) or press enter to continue with 0.0005: ')
if not param['varepsilon1']:
    param['varepsilon1'] = 0.0005  # SVD
else:
    param['varepsilon1'] = float(param['varepsilon1'])

param['varepsilon2'] = None # input('Introduce second tolerance (DMD-d Modes) or press enter to continue with 0.0005: ')
...
param['d'] = '/3' # input('Introduce parameter of DMD-d (d) or press enter to continue with 1 (options: a number; /3 or /5): ')
if not param['d']:
    param['d'] = 1
...
if os.path.isfile(param['metadata_path']) and param['metadata_path'].lower().endswith(('.xlsx','.xls')):
    df_metadata = pd.read_excel(param['metadata_path'])
...
for idx_class, class_name in enumerate(list_class_names):
    ...
    for idx_sequence, sequence_name in enumerate(list_sequences_class):
        ...
        #Buscar ese nombre  sequence_name_extracted  en la tabla de metadatos y coger el valor del FrameTime
        #param[deltaT] = FrameTime/1000
        
        rowmatch = df_metadata[df_metadata['Basename'] == sequence_name_extracted]
        
        if pd.notna(rowmatch.iloc[0]['FrameTime']) and isinstance(rowmatch.iloc[0]['FrameTime'], (float, int)):
            param['deltaT'] = rowmatch.iloc[0]['FrameTime']
        else:
            param['deltaT'] = 0.004
            
        if pd.notna(rowmatch.iloc[0]['Frames']) and isinstance(rowmatch.iloc[0]['Frames'], (int,float)) and rowmatch.iloc[0]['Frames'] <100:
            continue
