...
__author__ = "Egoitz Maiora Otaegi and David Oramas Muñoz"

"""
Perform DMD-d reduction to different input data types.
...
# @nb.njit(cache=True)
def truncatedSVD(A,esvd, svd_library = 'numpy', svd_driver = None):
    ...
    if svd_library == 'torch':
        ...
        if not(device.type == 'cpu'):
            ...
            U, s, Wh = torch.linalg.svd(torch.tensor(A).to(device), full_matrices=False, driver = svd_driver)
            ...
            U, s, Wh = torch.linalg.svd(torch.tensor(A).to(device), full_matrices=False)

    if svd_library == 'torch_new':

        # None, gesvd, gesvdj, and gesvda.
        U, s, Wh = torch.svd(torch.tensor(A).to(device))
    ...
    if svd_library == 'torch_32f':
        ...
        U, s, Wh = torch.linalg.svd(torch.tensor(A.astype(np.float32)).to(device), full_matrices=False)
        ...
        Wh = Wh.to(torch.float64)

    if svd_library == 'tensorflow':

        s, U, Wh = tf.linalg.svd(A)
    ...
    return U[:,:n],s[:n],Wh[:n,:], svd_proc_time
...
def truncHOSVD(A,esvd, svd_library = 'numpy', svd_driver = None):
    '''Decomposition into singular values for tensors, truncated in esvd'''
    Ulist=[]
    slist=[]
    S=A.copy()

    total_svd_proc_time = 0

    for i in range(A.ndim):
        [U,s,Vh, svd_proc_time]=truncatedSVD(unfold(A,i),esvd/np.sqrt(A.ndim), svd_library, svd_driver)

        total_svd_proc_time = total_svd_proc_time + svd_proc_time
    ...
    return S,Ulist,slist, total_svd_proc_time

def dmd1(V,t,esvd,edmd, svd_library= 'numpy', svd_driver = None):
    ...
    total_dmd1_time = time.time()

    total_svd_dmd1_time = 0

    dt=t[1]-t[0]

    #Reduced snapshots:
    U,s,Wh, svd_proc_time=truncatedSVD(V,esvd, svd_library, svd_driver) #I*r', r'*r', r'*J

    total_svd_dmd1_time = total_svd_dmd1_time + svd_proc_time
...
#    Vvir=s[:n]*Wh[:n,:]
    ...
    if svd_library == 'torch':
        ...
        if not(device.type == 'cpu'):
            ...
            Uvir, svir, Wvirh = torch.linalg.svd(torch.tensor(Vvir[:, :-1]).to(device), full_matrices=False, driver = svd_driver)
            Uvir = Uvir.detach().cpu()
            svir = svir.detach().cpu()
            Wvirh = Wvirh.detach().cpu()


        else:

            Uvir, svir, Wvirh = torch.linalg.svd(torch.tensor(Vvir[:, :-1]).to(device), full_matrices=False)

    if svd_library == 'torch_new':

        Uvir,svir,Wvirh = torch.svd(torch.tensor(Vvir[:, :-1]).to(device))
    ...
    if svd_library == 'torch_32f':
        ...
        Uvir,svir,Wvirh = torch.linalg.svd(torch.tensor(Vvir[:,:-1].astype(np.float32)).to(device), full_matrices=False)
        ...
        Uvir = Uvir.to(torch.float64)
        svir = svir.to(torch.float64)
        Wvirh = Wvirh.to(torch.float64)

    if svd_library == 'tensorflow':

        svir, Uvir, Wvirh = tf.linalg.svd(Vvir[:, :-1])

        Uvir = Uvir.numpy()
        svir = svir.numpy()
        Wvirh = Wvirh.numpy()

    svd_1_proc_time = time.time() - svd_1_proc_time

    total_svd_dmd1_time = total_svd_dmd1_time + svd_1_proc_time
    ...
    if svd_library == 'torch':
        ...
        if not (device.type == 'cpu'):
            ...
            Ua, sa, Wa = torch.linalg.svd(torch.tensor(A).to(device), full_matrices=False, driver = svd_driver)
            ...
            Ua, sa, Wa = torch.linalg.svd(torch.tensor(A).to(device), full_matrices=False)

    if svd_library == 'torch_new':

        Ua, sa, Wa = torch.svd(torch.tensor(A).to(device))
    ...
    if svd_library == 'torch_32f':
        ...
        Ua, sa, Wa = torch.linalg.svd(torch.tensor(A.astype(np.float32)).to(device),full_matrices=False)
        ...
        Ua = Ua.to(torch.float64)
        sa = sa.to(torch.float64)
        Wa = Wa.to(torch.float64)


    if svd_library == 'tensorflow':

        sa, Ua, Wa = tf.linalg.svd(A)

        Ua = Ua.numpy()
        sa = sa.numpy()
        Wa = Wa.numpy()

    svd_2_proc_time = time.time() - svd_2_proc_time

    total_svd_dmd1_time = total_svd_dmd1_time + svd_2_proc_time
...
#        areal[i]=np.linalg.norm(u[:,i])/np.sqrt(V.shape[1])
    ...
    total_dmd1_time = time.time() - total_dmd1_time

    return u,areal[mask],eigval[mask],delta[mask],omega[mask],DMDmode, total_svd_dmd1_time, total_dmd1_time

#@nb.njit(cache=True)
def hodmd(V,d,t,esvd,edmd, svd_library = 'numpy', svd_driver = None):
    ...
    #Reduced snapshots:
    U,s,Wh, svd_proc_time=truncatedSVD(V,esvd, svd_library, svd_driver) #I*n, n*n, n*J

    total_svd_time = total_svd_time + svd_proc_time
...
#    Vvir=s*Wh
    ...
    Udot,sdot,Whdot, svd_proc_time=truncatedSVD(Vdot,esvd, svd_library, svd_driver) #(d*n)*N, N*N, N*(J-d+1)
    ...
    if svd_library == 'torch':
        ...
        if not(device.type == 'cpu'):
            ...
            Uvd, svd, Whvd = torch.linalg.svd(torch.tensor(Vvd[:, :-1]).to(device), full_matrices=False, driver = svd_driver)

            Uvd = Uvd.detach().cpu()
            svd = svd.detach().cpu()
            Whvd = Whvd.detach().cpu()

        else:

            Uvd, svd, Whvd = torch.linalg.svd(torch.tensor(Vvd[:, :-1]).to(device), full_matrices=False)


    if svd_library == 'torch_new':

        # None, gesvd, gesvdj, and gesvda.
        Uvd, svd, Whvd = torch.svd(torch.tensor(Vvd[:, :-1]).to(device))
    ...
    if svd_library == 'torch_32f':
        ...
        Uvd, svd, Whvd = torch.linalg.svd(torch.tensor(Vvd[:, :-1].astype(np.float32)).to(device), full_matrices=False)
        ...
        Uvd = Uvd.to(torch.float64)
        svd = svd.to(torch.float64)
        Whvd = Whvd.to(torch.float64)

    if svd_library == 'tensorflow':

        svd, Uvd, Whvd = tf.linalg.svd(Vvd[:, :-1])
    ...
    total_svd_time = total_svd_time + svd_1_proc_time
...
#    print(A[:Uvir.shape[0],:])
    ...
    total_svd_time = total_svd_time + svd_2_proc_time
...
#    print(eigval)
    ...
    total_hodmd_it_time = time.time() - total_hodmd_it_time

    return u,areal[mask],eigval[mask],delta[mask],omega[mask],DMDmode,total_svd_time,total_hodmd_it_time

def tensorHODMD(V,d,t,esvd,edmd, svd_library = 'numpy', svd_driver = None):
    ...
    #Reduced snapshots:
    S,Vl,sl,total_hosvd_proc_time=truncHOSVD(V,esvd, svd_library, svd_driver) #I*n, n*n, n*J
    Svir=S.copy()
...
#    Vl[-1]=(sl[-1]*Vl[-1]).T
    ...
    uvir,a,eigval,delta,omega,DMDmode,total_svd_time,total_hodmd_it_time=hodmd((sl[-1]*Vl[-1]).T,d,t,esvd,edmd, svd_library, svd_driver)
    u=tensormatmul(Svir,uvir.T,V.ndim-1)

    return u,a,eigval,delta,omega,DMDmode,total_svd_time, total_hosvd_proc_time, total_hodmd_it_time
...
def hodmd_IT(Vvir,d,t,esvd,edmd, svd_library = 'numpy', svd_driver = None):
    '''High order (d) modal decomposition:
        Input:
            -V (IxJ): snapshot matrix.
            -t (J): time vector.
    ...
    # Vvd=np.diag(sdot)@Whdot #N*(J-d+1) ORIGINAL
    Vvd = np.matmul(np.diag(sdot), Whdot)

    #print("Size Vvd: ",Vvd.shape)

    #Spatial dimension reduction:
    # print(f'Spatial dimension reduction: {np.size(sdot)}') # ORIGINAL
    print(f'Spatial dimension reduction: {len(sdot)}')
    ...
    if svd_library == 'torch_32f':
        ...
        # Uvd, svd, Whvd = torch.linalg.svd(torch.tensor(Vvd[:, :-1].to(torch.float32)).to(device), full_matrices=False)
...
#    print(A[:Uvir.shape[0],:])
    ...
    if svd_library == 'torch_32f':
        ...
        if A.dtype == 'complex128':
            ...
            #Ua, sa, Wa = torch.linalg.svd(torch.tensor(A.astype('complex64')).to(device), full_matrices=False)
    ...
    if (svd_library == 'numpy'):
        ...
        a=Wa.conj().T@np.diag(sa**-1)@Ua.conj().T@b #N # ORIGINAL
    ...
    if (svd_library == 'torch') or (svd_library == 'torch_32f') or (svd_library == 'torch_new'):

        a=Wa.conj().T.resolve_conj().numpy()@np.diag(sa**-1)@Ua.conj().T.resolve_conj().numpy()@b #N
...
#    print(eigval)
    ...
    return u[:,0:kk3],areal[0:kk3],eigval[0:kk3],delta[0:kk3],omega[0:kk3], total_svd_time, total_hodmd_it_time
